...
def timestemp(typeTimestemp:str):
    ...
    while True:
        ...
        for(x,y,w,h) in faces:
            ...
            cv2.putText(image_frame, str(face_name), (x,y-40), font, 1, (255,255,255), 3)
            
            #savedatabase
            check = mg.checkTimestemp(face_id[0],typeTimestemp,datetime.datetime.now())
            #print(check)
            
            if check is 0 and face_name is not "Unknow":
                _id = mg.insertTimestemp(face_id[0],typeTimestemp).inserted_id
                cv2.putText(image_frame, "Check time "+typeTimestemp+" : OK" , (300,400), font, 1, (0,255,0),2)
                cv2.imwrite("image/"+str(_id)+'.jpg', image_frame)
            elif face_name is not "Unknow":
                cv2.putText(image_frame, "Check time "+typeTimestemp+" : OK" , (300,400), font, 1, (0,255,0),2)
